...
import com.kisanswap.kisanswap.functions.loadImageFromUrlAsBitmap
...
class EditProductViewModel(private val firestoreRepository: FirestoreRepository) : ViewModel() {
//    var product by mutableStateOf<Product?>(null)
    ...
    fun updateProductDetails(productId: String, updatedFields: Map<String, Any>, onComplete: (Boolean) -> Unit) {
        firestoreRepository.updateProductDetails(productId, updatedFields) { success ->
            if (success) {
                _currentProduct.value?.let { product ->
                    updatedFields.forEach { (key, value) ->
                        when (key) {
                            ...
                            "category" -> product.category = value as String
                        }
                    }
...
//                        val successs = firestoreRepository.updateProduct(currentProduct.value)
                    }
    ...
    fun addPhotoToProduct(productId: String, photoUrl: String, onComplete: (Boolean) -> Unit) {
        firestoreRepository.addPhotoToProduct(productId, photoUrl) { success ->
            if (success) {
    ...
    fun deletePhotoFromProduct(productId: String, photoUrl: String, onComplete: (Boolean) -> Unit) {
        firestoreRepository.deletePhotoFromProduct(productId, photoUrl) { success ->
            if (success) {
...
//                CacheManager.removePhoto(photoUrl)
    ...
    fun addVideoToProduct(productId: String, videoUrl: String, onComplete: (Boolean) -> Unit) {
        firestoreRepository.addVideoToProduct(productId, videoUrl) { success ->
            if (success) {
    ...
    fun deleteVideoFromProduct(productId: String, videoUrl: String, onComplete: (Boolean) -> Unit) {
        firestoreRepository.deleteVideoFromProduct(productId, videoUrl) { success ->
            if (success) {
...
//                CacheManager.removeVideo(videoUrl)
    ...
    fun updateProduct(onComplete: (Boolean) -> Unit) {
        ...
        viewModelScope.launch {
            ...
            firestoreRepository.updateProductDetails(currentProduct.value?.id ?: "", changes) { success ->
                onComplete(success)
            }
        }
    }
    ...
    /*fun fetchProductDetails(productId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            firestoreRepository.getProductDetails(productId) { fetchedProduct ->
    ...
    /*fun updateProduct(context: Context, onComplete: (Boolean) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            updatedProduct?.let {
                firestoreRepository.updateProduct(it) { success ->
                    onComplete(success)
                }
            }
        }
