...
import com.kisanswap.kisanswap.functions.saveVideoMetadata
...
class BuyViewModel(private val firestoreRepository: FirestoreRepository) : ViewModel() {
...
//                        _cachedVideoUri.value = currentList
    ...
    fun saveProductToUser(productId: String, onComplete: (Boolean) -> Unit) {
        ...
        firestoreRepository.saveProductToUser(userId, productId, onComplete)
    }
        ...
        fun addProductToCart(productId: String, onComplete: (Boolean) -> Unit) {
            firestoreRepository.saveProduct(productId) { success ->
                onComplete(success)
            }
        }

    fun incrementWhatsappDm(productId: String){
        firestoreRepository.incrementWhatsappDM(productId)
    }

    fun incrementCall(productId: String){
        firestoreRepository.incrementCall(productId)
    }

    fun incrementMapSearch(productId: String){
        firestoreRepository.incrementMapSearched(productId)
    }
